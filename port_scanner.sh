...
scan_ports_tcp() {
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ -z "$open_ports" ]; then
            echo "No open TCP ports found on $target." >> "$report_file"
            echo "No open TCP ports found on $target."
        else
            echo "Open TCP ports on $target: $open_ports" >> "$report_file"
            echo "Open TCP ports on $target: $open_ports"
        fi

        if [ -z "$closed_ports" ]; then
            echo "No closed TCP ports found on $target." >> "$report_file"
            echo "No closed TCP ports found on $target."
        else
            echo "Closed TCP ports on $target: $closed_ports" >> "$report_file"
...
scan_ports_udp() {
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ -z "$open_ports" ]; then
            echo "No open UDP ports found on $target." >> "$report_file"
            echo "No open UDP ports found on $target."
        else
            echo "Open UDP ports on $target: $open_ports" >> "$report_file"
            echo "Open UDP ports on $target: $open_ports"
        fi

        if [ -z "$closed_ports" ]; then
            echo "No closed UDP ports found on $target." >> "$report_file"
            echo "No closed UDP ports found on $target."
        else
            echo "Closed UDP ports on $target: $closed_ports" >> "$report_file"
